...
//    public static void dumbellsCombinations(HashMap<Float, Integer> dumbells, List<Float> combinations, List<Integer> left, List<Integer> right) {
//        float totalSum = left.stream().mapToDouble(Float::floatValue).sum();
//        float rightSum = Arrays.stream(left).sum();
//
//        if (leftSum == rightSum) {
//            combinations.add(leftSum+rightSum);
//        }
//
//        for(float peso: dumbells.keySet()) {
//            if (dumbells.get(peso) > 0) {
//                left
//            }
//        }
//    }
